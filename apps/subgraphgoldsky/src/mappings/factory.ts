...
export function handleNewPair(event: PairCreated): void {
  ...
  if (factory === null) {
    factory = new UniswapFactory(Bytes.fromHexString(FACTORY_ADDRESS))
    factory.pairCount = 0
    ...
    // Do not create Bundle here - it will be created by pricing.ts when first needed
  }
  ...
  if (token0 === null) {
    ...
    // Initialize archive helper fields
  ...
  if (token1 === null) {
    ...
    // Initialize archive helper fields
    token1.lastMinuteArchived = ZERO_BI
    token1.lastHourArchived = ZERO_BI
    token1.minuteArray = []
    token1.hourArray = []
    token1.lastMinuteRecorded = ZERO_BI
    token1.lastHourRecorded = ZERO_BI
  }
