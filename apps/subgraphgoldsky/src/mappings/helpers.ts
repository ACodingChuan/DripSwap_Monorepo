...
export function createLiquiditySnapshot(position: LiquidityPosition, event: ethereum.Event): void {
  ...
  // If bundle doesn't exist yet, we can't create a snapshot with accurate USD prices
  // This typically happens during initial sync before any Sync events are processed
  if (bundle === null) {
    log.warning('Bundle not found when creating liquidity snapshot at block {}', [event.block.number.toString()])
    return
  }
  ...
  // create new snapshot
  // Use tx hash + logIndex to avoid ID collisions when multiple events happen within the same second.
  let snapshotId = position.id.concat(event.transaction.hash).concatI32(event.logIndex.toI32())
